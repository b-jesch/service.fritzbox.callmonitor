...
#import keyring
# uses windows specific stuff

KEYRING_SYSTEM = 'pyicloud://icloud-password'


def get_password(username, interactive=False):#sys.stdout.isatty()):    #isatty -> not usable for xbmcout
    try:
...
def get_password_from_keyring(username):
    result = None#keyring.get_password(
        #KEYRING_SYSTEM,
        #username
    #)
    if result is None:
